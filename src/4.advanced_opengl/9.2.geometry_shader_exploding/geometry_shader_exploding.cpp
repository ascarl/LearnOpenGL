// LearnOpenGL 中文导读
// 学习目标：让几何 Shader 根据裁剪空间 xyz 边向量的叉积构造风格化方向，形成周期性爆炸效果。
// 核心流程：顶点阶段先完成 MVP 变换，几何阶段从已投影位置求位移方向，并发射加过裁剪空间偏移的三个顶点。
// 观察重点：该方向不是真实几何面法线；各顶点 w 可不同，相同裁剪空间偏移不保证透视除法后的刚性平移。
